...
export async function fetchPublicationsPreview(orcid) {
    try {
        ...
        console.log("Publications fetched for preview:", publications)
...
export async function addManualPublication(formData) {
    try {
        ...
        const impactFactor = Number.parseFloat(formData.get("impactFactor")) || 0
        const year = formData.get("year")
        const authors = formData.get("authors")
        const doi = formData.get("doi")
        const url = formData.get("url")
        const osfLink = formData.get("osfurl")
        const abstract = formData.get("abstract")
        const supplementaryMaterials = formData.get("supplementaryMaterials") || null
