    ...
    try {
        ...
        const results = await Promise.allSettled(
            publicationIds.map(async (publicationId) => {
                const { error } = await deleteUserPublication(publicationId, userId)
                if (error) {
                    throw new Error(`Erreur pour ${publicationId}: ${error}`)
                }
        ...
        if (failed > 0) {
            return {
                success: false,
                error: `${successful} publication(s) supprimée(s), ${failed} échec(s)`,
            }
        }

        return {
            success: true,
            message: `${successful} publication(s) supprimée(s) avec succès`,
        }
    } catch (error) {
...
export async function deletePublicationPDF(publicationId) {
    try {
        ...
        await admin.db.collection("publications").doc(publicationId).update({
            ...
            pdfFileName: admin.firestore.FieldValue.delete(),
            pdfStoragePath: admin.firestore.FieldValue.delete(),
            pdfUploadedAt: admin.firestore.FieldValue.delete(),
        })
