import { doc, updateDoc, collection, addDoc, getDocs, query, where, orderBy, deleteDoc } from "firebase/firestore"
...
export const getUserPublications = async (uid) => {
    try {
        const publicationsRef = collection(db, "publications")
        const q = query(publicationsRef, where("userId", "==", uid), orderBy("year", "desc"))
